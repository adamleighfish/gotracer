...
func CreateScene() *HitableList {
	var list HitableList = HitableList{}
        ...
        checker := &CheckeredTexture{&ConstTexture{Vector{0.2, 0.3, 0.1}}, &ConstTexture{Vector{0.9, 0.9, 0.9}}}

	// create and add the floor
        floor := Sphere{Vector{0.0, -1000.0, 0}, 1000.0, &Lambertian{checker}}
        list.Add(&floor)
	...
	for a := -10; a < 10; a++ {
		for b := -10; b < 10; b++ {
			...
			if center.Subtract(Vector{4.0, 0.2, 0.0}).Length() > 0.9 {
                                ...
                                if chooseMat < 0.7 {
                                        list.Add(&Sphere{center, 0.2, &Lambertian{&ConstTexture{Vector{
				...
				// choose moving diffuse material
         				...
         				list.Add(&MovingSphere{center, center.Add(Vector{0.0, 0.5 * rand.Float64(), 0.0}), 0.0, 1.0, 0.2,
                                                &Lambertian{&ConstTexture{Vector{
                                                X: rand.Float64() * rand.Float64(),
						Y: rand.Float64() * rand.Float64(),
						Z: rand.Float64() * rand.Float64()}}}})
				// choose metal material
				} else if chooseMat < 0.95 {
					list.Add(&Sphere{center, 0.2, &Metal{&ConstTexture{Vector{
	...
	sphere2 := Sphere{Vector{-4.0, 1.0, 0.0}, 1.0, &Lambertian{&ConstTexture{Vector{0.4, 0.2, 0.1}}}}
	sphere3 := Sphere{Vector{4.0, 1.0, 0.0}, 1.0, &Metal{&ConstTexture{Vector{0.7, 0.6, 0.5}}, 0.0}}
